    ...
    if st.session_state.current_llm_output:
        with st.container():
            ...
            <div class="llm-output-container">
                <div class="llm-output-header">LLM Grading Output</div>
                <div class="llm-output-content">
            """, unsafe_allow_html=True)
            st.json(st.session_state.current_llm_output)
    ...
    with st.sidebar:
        ...
        if provider != "Local Only":
            ...
            if st.button("🔄 Refresh Available Models"):
                if api_key:
                    # Store API key in session state and env
            ...
            # Only show model selection if we have an API key
